...
\begin{itemize}
    \item \textbf{Parâmetros de Configuração:} A função \texttt{validate\_config\_params()} verifica se os parâmetros do sistema estão em intervalos válidos: capacidade $\eta > 0$ e $\eta \leq$ MAX\_CAPACITY, velocidade $\gamma > 0$, tempo máximo de espera $\delta \geq 0$, distâncias máximas $\alpha, \beta \geq 0$, e eficiência mínima $\lambda \in [0,1]$. Valores fora desses intervalos causam encerramento controlado do programa com mensagem de erro descritiva.
    
    \item \textbf{Validação de Demandas:} A função \texttt{validate\_demand()} verifica cada solicitação individualmente, garantindo que coordenadas sejam válidas, que origem e destino sejam diferentes (distância $> 0.01$), e que o tempo de solicitação seja não-negativo. Demandas inválidas são descartadas com aviso, permitindo que o sistema continue processando as demais.
    
    \item \textbf{Contagem de Demandas:} A função \texttt{validate\_demand\_count()} verifica se o número de demandas é positivo e emite aviso se exceder 100.000, indicando possível problema de desempenho.
\end{itemize}

\subsection{Verificação de Alocação de Memória}

Todas as chamadas a \texttt{malloc()} foram instrumentadas com verificação de retorno NULL:

\begin{itemize}
    \item \texttt{create\_new\_ride()}: Verifica alocação de estrutura Ride
    \item \texttt{initialize()}: Verifica alocação do array de paradas do MinHeap
    \item \texttt{create\_new\_stop()}: Verifica alocação de cada parada individual
\end{itemize}

Em caso de falha de alocação, o sistema imprime mensagem de erro detalhada e encerra com \texttt{exit(1)}, prevenindo segmentation faults por acesso a ponteiros NULL.
...
\begin{itemize}
    \item \texttt{calculate\_stop\_time()}: Verifica se velocidade $> 0$ antes de calcular tempo = distância/velocidade
    \item \texttt{calculate\_ride\_efficiency()}: Verifica se distância total $> 0.0001$ (tolerância para comparação de floats) antes de calcular eficiência = distância\_original/distância\_total
\end{itemize}
...
Implementada proteção contra buffer overflow no array \texttt{demands[MAX\_CAPACITY]}:

\begin{itemize}
    \item \texttt{add\_ride\_demand()}: Verifica se \texttt{demand\_number < MAX\_CAPACITY} antes de adicionar
    \item \texttt{remove\_last\_added\_demand()}: Verifica se \texttt{demand\_number > 0} antes de remover
    \item \texttt{meets\_distance\_criteria()}: Verifica se \texttt{demand\_number $\leq$ MAX\_CAPACITY} antes de iterar
\end{itemize}
...
Estas validações são chamadas em pontos críticos (antes de finalizar o heap) e ajudam a detectar bugs de lógica durante desenvolvimento e testes.
    ...
    \item \textbf{Invariantes Temporais:} A função \texttt{validate\_ride\_temporal\_consistency()} verifica que os tempos de paradas são monotonicamente crescentes ao longo da corrida.
    
    \item \textbf{Invariantes do Heap:} A função \texttt{validate\_heap\_times()} verifica que todos os tempos no MinHeap são não-negativos, e \texttt{is\_valid\_minheap()} verifica a propriedade de heap mínimo.
\end{itemize}

Estas validações são chamadas em pontos críticos (após adicionar/remover demandas, antes de finalizar o heap) e ajudam a detectar bugs de lógica durante desenvolvimento e testes.
